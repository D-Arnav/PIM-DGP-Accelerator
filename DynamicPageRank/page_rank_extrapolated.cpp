...
public:
    ...
    void full_recompute(int max_iter = 100) {
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            rank = nr;
            if (diff < epsilon) {
                std::cout << "  Converged after " << (iter+1) << " iterations (diff=" << diff << ")\n";
                break;
            }
        }
...
void bench_speed(DynamicPageRank& dpr,
    ...
    std::mt19937 rng(42);
    std::uniform_int_distribution<int> dist(0, (int)all_edges.size()-1);
    std::vector<std::pair<int,int>> test_edges;
    for (int i = 0; i < n_updates; ++i)
        test_edges.push_back(all_edges[dist(rng)]);

    Timer t;
    t.start();
    for (auto [u,v] : test_edges) { dpr.add_edge(u,v); dpr.remove_edge(u,v); }
    double dynamic_ms = t.ms();

    t.start();
    dpr.full_recompute();
    double one_recompute_ms = t.ms();
    double full_ms = one_recompute_ms * n_updates;

    std::cout << std::fixed << std::setprecision(2);
    std::cout << "  Updates tested          : " << n_updates << " add+remove pairs\n";
    std::cout << "  Dynamic total           : " << dynamic_ms << " ms\n";
    std::cout << "  Full recompute x" << n_updates << "  : " << full_ms << " ms  (extrapolated)\n";
    std::cout << "  Speedup                 : " << (full_ms / dynamic_ms) << "x\n";
    std::cout << "  Per dynamic update      : " << (dynamic_ms / n_updates) << " ms\n";
    std::cout << "  Per full recompute      : " << one_recompute_ms << " ms\n";
}
...
void bench_memory(DynamicPageRank& dpr) {
    ...
    if (rss > 0)
        ...
        std::cout << "    Process RSS     : unavailable\n";
